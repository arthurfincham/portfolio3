...
export default function ProjectItem({ project, className, style, addImage }) {
  ...
  const Link = ({ url, text, testID }) => {
    return (
      <div className="h-auto p-1 px-2 rounded-t-lg shadow-lg bg-white hover:bg-gray-100 dark:bg-gray-700">
        <a
          href={url}
          target="_blank"
          className="text-sm dark:text-white font-a3"
          data-testid={testID}
          rel="noreferrer"
        >
          {text}
  ...
  return (
    <div style={style} className="overflow-hidden rounded-lg shadowTop fcc">
      <div className="relative items-end justify-between w-full px-4 pt-2 overflow-hidden rounded-t-lg shadowTop bg-white fr dark:bg-gray-900/40">
        <div className="space-x-2 fr ">
          <Link
            url={project.repoURL}
            text="GitHub"
            testID={`${project.title}-GitHubLink`}
          />
          {project.liveLink ? (
            <Link
              url={project.liveLink}
              text="Site"
              testID={`${project.title}-LiveSiteLink`}
            />
          ) : (
            ''
          )}
        ...
        <div className="items-center space-x-1 fr">
          {isPinned ? (
            <PinIcon styles={pinStyle} className="w-[15px] h-[15px] mb-1 " />
          ) : (
            ''
          )}
          <span className="text-xl font-a1 dark:text-white">
            {project.title}
          </span>
        </div>
      </div>
      ...
      >
        ...
        <animated.div
          style={infoStyle}
          className="absolute bottom-0 w-full overflow-hidden shadow-lg bg-white dark:bg-gray-800 dark:text-white"
        >
            ...
            <div className="fr">
              {project.stack.map((tool, index) => {
                return (
                  <div
                    key={index}
                    className="items-center justify-center w-full px-2 space-x-2 fr"
                  >
                    {mobile ? '' : iconSelector(tool, 'w-1/5 h-auto')}
                    <span className="text-xs sm:text-sm font-a3">
                      {capitalize(tool)}
                    </span>
                  </div>
                );
